...
#
#   # for add, shape of lhs and rhs should be identical. for all other strategy ops, we take shape of "data" input arg
...
"""
Helper functions for parsing strategy op name annotation
e.g., 0-tensorrt_conv2d+relu -> 1, 0-tvm_conv2d_+relu -> 0
"""
def create_strategy_pattern_annotation(group_id, annotation):
  return f"{group_id}-{annotation}"

def get_group_id_from_strategy_pattern_annotation(annotation):
  return annotation.split("-")[0]

def get_strategy_pattern_name_from_strategy_pattern_annotation(annotation):
  strategy_pattern_name_pos = annotation.find("-")+1
  return annotation[strategy_pattern_name_pos:]

def get_strategy_from_strategy_pattern_annotation(annotation):
  return get_strategy_pattern_name_from_strategy_pattern_annotation(annotation).split("_")[0]

def get_op_name_from_strategy_pattern_annotation(annotation):
  op_name_pos = annotation.find("_") + 1
  return annotation[op_name_pos:]

def get_group_id_and_strategy_pattern_name(annotation):
  return get_group_id_from_strategy_pattern_annotation(annotation), get_strategy_pattern_name_from_strategy_pattern_annotation(annotation)
...
# Note that expr is extracted subgraph that matches strategy op
